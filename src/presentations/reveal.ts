import type { PresentationAssetResolver } from "./assets.js";
import type { PresentationDeck } from "./schema.js";
import { renderDeckSlide } from "./reveal-slide-renderer.js";
...
${deck.slides.map((slide, index) => renderDeckSlide(deck, slide, index, false, { assetResolver: options.assetResolver, editable })).join("\n")}
</main>
...
export function renderStaticSlideHtml(deck: PresentationDeck, slideIndex = 0, options: CompilePresentationOptions = {}): string {
  ...
  if (!slide) return "";
  return `<div class="presentation-static"><style>${presentationCss(deck)}</style>${renderDeckSlide(deck, slide, slideIndex, true, { assetResolver: options.assetResolver, editable: options.editable })}</div>`;
}
