...
import { parseSessionTranscript, type ParsedSessionTranscript } from "./session-transcript-parser.js";
...
export class SessionSearchService {
  ...
  private async syncImpl(): Promise<void> {
    ...
    for (const sessionFile of files) {
      ...
      const parsed = await parseSessionTranscript(sessionFile);
  ...
  private replaceFile(sessionFile: string, stat: import("node:fs").Stats, parsed: ParsedSessionTranscript): void {
    this.deleteFile(sessionFile);
    this.db.exec("BEGIN");
    try {
