...
test("presentation artifact renders preview and present modal", async ({ page }) => {
  ...
  await expect(page.getByText("QXO Signal Brief").first()).toBeVisible();
  ...
  expect(download.suggestedFilename()).toBe("qxo-signal-brief.html");
  ...
  await expect(page.getByRole("dialog", { name: /QXO Signal Brief presentation/ })).toBeVisible();
  ...
  await expect(page.getByRole("dialog", { name: /QXO Signal Brief presentation/ })).toHaveCount(0);
});
